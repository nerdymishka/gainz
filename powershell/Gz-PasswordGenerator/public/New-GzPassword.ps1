function New-GzPassword() {
    ...
    if(!$CharSets -and $CharSets.Length -gt 0) {
        $set = (Merge-GzPasswordCharSets $CharSets)
    ...
    if($sb.Length -eq 0) {
        $sets = Merge-GzPasswordCharSets -CharSets (@('LatinAlphaUpperCase', 'LatinAlphaLowerCase', 'Digits', "SpecialHybrid"))
    ...
    while( (Test-GzPassword $password -Validate $Validate) -eq $false) {
