function Merge-NuspecConfig() {
    ...
    $left | Get-Member -MemberType NoteProperty  | ForEach-Object {
        ...
        if($null -eq $rightValue -and $leftValue -is [System.Management.Automation.PSCustomObject]) {
             $rightValue = New-Object PSObject 
             $rightValue = Merge-NuspecConfig -Source $leftValue -Destination $rightValue
