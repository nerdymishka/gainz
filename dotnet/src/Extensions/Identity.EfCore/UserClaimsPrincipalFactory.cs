...
namespace NerdyMishka.Identity {
        ...
        where TUser : class 
    {
        ...
        /// </value>
        public UserClaimsPrincipalFactory (IdentityOptions options) {
        ...
        protected virtual async Task<ClaimsIdentity> GenerateClaimsAsync (TUser user) {
            ...
            if (!id.HasClaim (o => o.Type == NerdyClaimTypes.Pseudonym))
                id.AddClaim (new Claim (NerdyClaimTypes.Pseudonym, userName.ToLowerInvariant()));
            ...
            if(UserManager.SupportsPermissionClaims)
                id.AddClaims(await this.UserManager.GetPermissionClaimsAsync(user));
            

            return id;
        }

        Task<ClaimsPrincipal> IUserClaimsPrincipalFactory<TUser>.CreateAsync (TUser user) {
